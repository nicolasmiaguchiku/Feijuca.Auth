        ...
        public async Task<Result<IEnumerable<ClientRoleResponse>>> Handle(GetRolesQuery request, CancellationToken cancellationToken)
        {
            ...
            if (result.IsSuccess)
            {
                ...
                foreach (var client in result.Data)
                {
                    ...
                    {
                        var rolesResponse = rolesResult.Data.Select(x => new RoleResponse(x.Id, x.Name, x.Description));
                        roleResponse.Add(new ClientRoleResponse(client.ClientId, rolesResponse));
                    }
                }
